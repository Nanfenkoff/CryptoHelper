...
function completeHash(oString, oKey) {
    ...
    for (var x = 0, y = ''; x < oString.length; x += Math.round(oKey.length / 5), y = ' ') {
        var theNum = parseInt('0x' + completeEscape(oString.substr(x, Math.round(oKey.length / 5))));
        if (isNaN(theNum)) {
            window.alert('FaiLhou gg');
            return oString;
        }
        for (var z = 0; z < oKey.length; z++) {
            eval('theNum ' + oOp[z % 1] + ' ' + oKeyNum[z] + ';');
        }
        oOutStr += y + theNum;
    }
    return oOutStr;
...
function unHash(oString, oKey) {
    ...
    for (var x = 0; x < oOutStr.length; x++) {
        ...
        for (var z = oKey.length - 1; z >= 0; z--) {
            eval('oOutStr[x] ' + oOp[z % 1] + ' ' + oKeyNum[z] + ';');
        }
