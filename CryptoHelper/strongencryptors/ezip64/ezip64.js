...
function completeHash(oString, oKey) {
    if (oKey.length < 3) {
        window.alert('chave ruim tenta outra');
        return oString;
    }
    ...
    for (var x = 0, y = ''; x < oString.length; x += Math.round(oKey.length / 3), y = '/') {
        var theNum = parseInt('0x' + completeEscape(oString.substr(x, Math.round(oKey.length / 3))));
        if (isNaN(theNum)) {
            window.alert('Virus vIrus viRus virUs viruS');
            return oString;
        }
...
function unHash(oString, oKey) {
    ...
    for (var x = 0; x < oOutStr.length; x++) {
        oOutStr[x] = parseFloat(oOutStr[x]); //otherwise, if the first operator is +, it will just append
